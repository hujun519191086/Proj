...
public class SystemNotification {
	...
	public static void showPopupWindow(Context context, View contentView, final View dropView, int anim, int width,
			...
			((AbsListView) contentView).setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					if (dropView instanceof TextView) {
						((TextView) dropView).setText(((TextView) view).getText().toString().replaceAll("[^\\d+]", ""));
					}
					pw.dismiss();
				}
			});
	...
	public static void showPopupWindow(Context context, View contentView, final View dropView, int width, int height,
			int xoff, int yoff) {
		showPopupWindow(context, contentView, dropView, 0, width, height, xoff, yoff);
	}
	 ...
	 * @param dropView 在哪个view下面显示
	 * @param width 要显示的view的宽
	 * @param height 要显示的view的高
	 * @param xoff x轴偏移量
	 * @param yoff y轴偏移量
	 * @param listener 如果显示的是listview，设置监听
	 */
	@SuppressWarnings("deprecation")
	public static void showPopupWindow(Context context, View contentView, View dropView, int width, int height,
			int xoff, int yoff, final OnItemClickListener listener) {
		ViewGroup g = ((ViewGroup) contentView.getParent());
		if (g != null) {
			g.removeView(contentView);
		}
